...
public class MemorySummaryAggregationTest {
	...
	static {
		default_aggregates = ImmutableMap.<String, Integer>builder().put("min", MemorySummaryMeta.TYPE_GROUP_MIN)
				.put("max", MemorySummaryMeta.TYPE_GROUP_MAX).put("sum", MemorySummaryMeta.TYPE_GROUP_SUM)
				.put("ave", MemorySummaryMeta.TYPE_GROUP_AVERAGE).put("count", MemorySummaryMeta.TYPE_GROUP_COUNT_ALL)
				.put("count_any", MemorySummaryMeta.TYPE_GROUP_COUNT_ANY)
				.put("count_distinct", MemorySummaryMeta.TYPE_GROUP_COUNT_DISTINCT).build();
	}
	...
	private RowMetaAndData runStep() throws KettleException {
		...
		MemorySummaryData data = new MemorySummaryData();
		...
		MemorySummary step = spy(new MemorySummary(stepMeta, data, 0, transMeta, mock(Trans.class)));
