...
import com.kaszub.pdi.memsummary.MemorySummary;
import com.kaszub.pdi.memsummary.MemorySummaryData;
import com.kaszub.pdi.memsummary.MemorySummaryData.HashEntry;

public class MemorySummaryAggregationNullsTest {

	static StepMockHelper<MemorySummaryMeta, MemorySummaryData> mockHelper;

	MemorySummary step;
	MemorySummaryData data;
	...
	public static void setUpBeforeClass() throws Exception {
		mockHelper = new StepMockHelper<MemorySummaryMeta, MemorySummaryData>("Memory Group By",
				MemorySummaryMeta.class, MemorySummaryData.class);
	...
	public void setUp() throws Exception {
		...
		data.groupMeta = new RowMetaInterface[]{rmi};
		...
		data.aggMeta = rmi;
		step = new MemorySummary(mockHelper.stepMeta, data, 0, mockHelper.transMeta, mockHelper.trans);
	...
	HashEntry getHashEntry() {
		return data.getHashEntry(new Object[data.groupMeta.length],data.groupMeta[0]);
	}
	 ...
	 * 
	 * Set this variable to Y to set the minimum to NULL if NULL is within an
	 * aggregate. Otherwise by default NULL is ignored by the MIN aggregate and
	 * MIN is set to the minimum value that is not NULL. See also the variable
	...
	/**
	 * Set this variable to Y to return 0 when all values within an aggregate
	 * are NULL. Otherwise by default a NULL is returned when all values are
	 * NULL.
	 * 
